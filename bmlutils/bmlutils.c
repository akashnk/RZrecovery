...
static int
restore_internal (const char *bml, const char *filename)
{
  char buf[4096];
...
int
cmd_bml_restore_raw_partition (const char *partition, const char *filename)
{
  if (strcmp (partition, "boot") != 0 && strcmp (partition, "recovery") != 0
      && strcmp (partition, "recoveryonly") != 0)
    return 6;
...
int
cmd_bml_backup_raw_partition (const char *partition, const char *out_file)
{
  char *bml;
...
int
cmd_bml_erase_raw_partition (const char *partition)
{
...
int
cmd_bml_erase_partition (const char *partition, const char *filesystem)
{
  return -1;
}

int
cmd_bml_mount_partition (const char *partition, const char *mount_point,
...
int
cmd_bml_get_partition_device (const char *partition, char *device)
{
  return -1;
}
