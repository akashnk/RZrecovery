...
Value *
MountFn(const char *name, State * state, int argc, Expr * argv[])
{
  char *result = NULL;
  if (argc != 4)
  {
...
Value *
IsMountedFn(const char *name, State * state, int argc, Expr * argv[])
{
  char *result = NULL;
  if (argc != 1)
  {
...
Value *
UnmountFn(const char *name, State * state, int argc, Expr * argv[])
{
  char *result = NULL;
  if (argc != 1)
  {
...
FormatFn(const char *name, State * state, int argc, Expr * argv[])
{
  ...
  char *location;
  char *fs_size;
  if (ReadArgs(state, argv, 4, &fs_type, &partition_type, &location, &fs_size)
      < 0)
  {
    return NULL;
  }
...
Value *
SetProgressFn(const char *name, State * state, int argc, Expr * argv[])
{
  if (argc != 1)
  {
    return ErrorAbort(state, "%s() expects 1 arg, got %d", name, argc);
...
Value *
PackageExtractFileFn(const char *name, State * state, int argc, Expr * argv[])
{
  if (argc != 1 && argc != 2)
  {
  ...
  else
  {
    ...
    if (ReadArgs(state, argv, 1, &zip_path) < 0)
      return NULL;
...
Value *
RetouchBinariesFn(const char *name, State * state, int argc, Expr * argv[])
{
  UpdaterInfo *ui = (UpdaterInfo *) (state->cookie);
...
Value *
UndoRetouchBinariesFn(const char *name, State * state,
		      int argc, Expr * argv[])
{
  UpdaterInfo *ui = (UpdaterInfo *) (state->cookie);
  ...
  while (i < (argc - 1))
  {
    success = success && retouch_one_library(retouch_entries[i],
					     ...
					     NULL);
    if (!success)
      ErrorAbort(state, "Failed to unretouch '%s'.", retouch_entries[i]);
...
Value *
SymlinkFn(const char *name, State * state, int argc, Expr * argv[])
{
  if (argc == 0)
  {
...
Value *
SetPermFn(const char *name, State * state, int argc, Expr * argv[])
{
  char *result = NULL;
  ...
  if (recursive)
  {
    ...
    if (*end != '\0' || args[3][0] == 0)
    {
      ErrorAbort(state, "%s: \"%s\" not a valid filemode", name, args[3]);
      goto done;
    }
...
Value *
FileGetPropFn(const char *name, State * state, int argc, Expr * argv[])
{
  char *result = NULL;
  char *buffer = NULL;
  char *filename;
...
static bool
write_raw_image_cb(const unsigned char *data, int data_len, void *ctx)
{
  int r =
    mtd_write_data((MtdWriteContext *) ctx, (const char *) data, data_len);
  if (r == data_len)
    return true;
  fprintf(stderr, "%s\n", strerror(errno));
  return false;
...
Value *
WriteRawImageFn(const char *name, State * state, int argc, Expr * argv[])
{
  char *result = NULL;
...
Value *
ApplyPatchSpaceFn(const char *name, State * state, int argc, Expr * argv[])
{
  char *bytes_str;
...
Value *
ApplyPatchFn(const char *name, State * state, int argc, Expr * argv[])
{
  if (argc < 6 || (argc % 2) == 1)
  {
...
Value *
ApplyPatchCheckFn(const char *name, State * state, int argc, Expr * argv[])
{
  if (argc < 1)
  {
...
Value *
UIPrintFn(const char *name, State * state, int argc, Expr * argv[])
{
  char **args = ReadVarArgs(state, argc, argv);
  if (args == NULL)
  {
...
Value *
WipeCacheFn(const char *name, State * state, int argc, Expr * argv[])
{
  if (argc != 0)
  {
...
//
Value *
Sha1CheckFn(const char *name, State * state, int argc, Expr * argv[])
{
  if (argc < 1)
  {
    return ErrorAbort(state, "%s() expects at least 1 arg", name);
  ...
  for (i = 1; i < argc; ++i)
  {
    ...
    {
      fprintf(stderr, "%s(): arg %d is not a string; skipping", name, i);
    }
...
Value *
ReadFileFn(const char *name, State * state, int argc, Expr * argv[])
{
  if (argc != 1)
  {
    return ErrorAbort(state, "%s() expects 1 arg, got %d", name, argc);
