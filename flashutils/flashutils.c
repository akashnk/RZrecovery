...
int
get_flash_type (const char *partitionType)
{
...
static int
detect_partition (const char *partitionType, const char *partition)
{
...
int
restore_raw_partition (const char *partitionType, const char *partition,
		       const char *filename)
{
...
int
backup_raw_partition (const char *partitionType, const char *partition,
		      const char *filename)
{
  int type = detect_partition (partitionType, partition);
...
int
erase_raw_partition (const char *partitionType, const char *partition)
{
  int type = detect_partition (partitionType, partition);
...
int
erase_partition (const char *partition, const char *filesystem)
{
...
int
mount_partition (const char *partition, const char *mount_point,
		 const char *filesystem, int read_only)
{
  ...
  switch (type)
	  {
	  case MTD:
	    return cmd_mtd_mount_partition (partition, mount_point,
					    filesystem, read_only);
	  case MMC:
	    return cmd_mmc_mount_partition (partition, mount_point,
					    filesystem, read_only);
	  case BML:
	    return cmd_bml_mount_partition (partition, mount_point,
					    filesystem, read_only);
	  default:
	    return -1;
	  }
...
int
get_partition_device (const char *partition, char *device)
{
  int type = device_flash_type ();
